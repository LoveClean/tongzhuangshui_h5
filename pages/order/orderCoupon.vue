<template>
	<view>
		<view class="tabr" :style="{ top: headerTop }">
			...
			<view :class="{ on: typeClass == 'invalid' }" @tap="switchType('invalid')">不可用</view>
		...
		<view class="list">
			...
			<view class="sub-list valid" :class="subState">
				...
				<view class="row" v-for="(row, index) in couponValidList" :key="index">
					<!-- content -->
					<view class="carrier" @tap="selected(row)">
						<view class="left">
						...
						<view class="right" :class="row.couponsScope === 4 ? '' : 'onlyShop'">
							<view class="ticket">
			...
			<view class="sub-list invalid" :class="subState">
				...
				<view class="row" v-for="(row, index) in couponinvalidList" :key="index">
					...
					<view class="carrier">
						<view class="left">
							<view class="title">{{ row.couponsName }}</view>
							<view class="notMsg">{{ row.effectiveDate }} ~ {{ row.expiryDate }}</view>
							<view class="notMsg">详细信息：{{ row.notMsg }}</view>
	...
	onLoad(option) {
		...
		uni.request({
			...
			success: res => {
				...
				this.couponValidList = data.unable;
				this.couponinvalidList = data.disable;
